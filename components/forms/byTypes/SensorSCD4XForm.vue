...
const schema = z.object({
  ...
  // Давление
  minAvailableCO2: z.number().nullable().default(0),
  maxAvailableCO2: z.number().nullable().default(0),
  minAlarmCO2: z.number().nullable().default(0),
  maxAlarmCO2: z.number().nullable().default(0),
  enableGraphingCO2: z.boolean(),
});
...
// CO2
const { value: minAvailableCO2 } = useField<FormSchema['minAvailableCO2']>('minAvailableCO2');
const { value: maxAvailableCO2 } = useField<FormSchema['maxAvailableCO2']>('maxAvailableCO2');
const { value: minAlarmCO2 } = useField<FormSchema['minAlarmCO2']>('minAlarmCO2');
const { value: maxAlarmCO2 } = useField<FormSchema['maxAlarmCO2']>('maxAlarmCO2');
const { value: enableGraphingCO2 } = useField<FormSchema['enableGraphingCO2']>('enableGraphingCO2');
...
const submitForm = (values: FormSchema) => {
  const dataToSave = {
    ...
    CO2Settings: {
      minAvailable: values.minAvailableCO2,
      maxAvailable: values.maxAvailableCO2,
      minAlarm: values.minAlarmCO2,
      maxAlarm: values.maxAlarmCO2,
      enableGraphing: values.enableGraphingCO2,
    },
    ports: {
      sda: values.sdaPort,
      scl: values.sclPort,
...
getControllersViaType(Sensor.SCD4X);
</script>
...
<template>
  <form class="tw-text-lg tw-font-semibold" @submit.prevent="handleSubmit(submitForm)">
    ...
    <!-- Давление -->
    <p class="tw-mb-4">{{ t('devices.co2') }}</p>

    <div class="custom-grid">
      <SharedUIFormLabel :label="t('devices.graphing')" />
      <ToggleSwitch v-model="enableGraphingCO2" />
    </div>

    <div class="tw-mb-2 tw-grid tw-grid-cols-[1.4fr_2fr_1.4fr_2fr] tw-gap-4">
      <SharedUIFormLabel :label="`${t('devices.minAvailability')}:`" :tooltip="t('devices.tooltipMinAvailability')" />
      <InputNumber v-model="minAvailableCO2" suffix=" ppm" />
      <SharedUIFormLabel :label="t('devices.maxAvailability')" />
      <InputNumber v-model="maxAvailableCO2" suffix=" ppm" />
    </div>

    <div class="tw-mb-4 tw-grid tw-grid-cols-[1.4fr_2fr_1.4fr_2fr] tw-gap-4">
      <SharedUIFormLabel :label="`${t('devices.minAlarm')}:`" :tooltip="t('devices.tooltipMinAlarm')" />
      <InputNumber v-model="minAlarmCO2" suffix=" ppm" />
      <SharedUIFormLabel :label="t('devices.maxAlarm')" />
      <InputNumber v-model="maxAlarmCO2" suffix=" ppm" />
    </div>
  </form>
</template>
